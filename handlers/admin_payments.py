...
from tgbot.services.liqpay import liqpay
from design import qiwi_text, yoomoney_text, lava_text, lzt_text, crystalPay_text , liqpay_text
...
"""
    ...
    if way == "qiwi":
        ...
        await call.message.edit_text(pay_info(ways, status), reply_markup=payments_settings_info(way, status))
         ...
         ways = liqpay_text
         status = get_payments()['pay_liqpay']
...
async def off_payments(call: CallbackQuery, state: FSMContext):
    ...
    if way == "qiwi":
        ...
        if action == "off":
            update_payments(pay_liqpay="False")
        else:
            update_payments(pay_liqpay="True")
...
async def payments_balance_call(call: CallbackQuery, state: FSMContext):
    ...
    if way == "qiwi":
        ...
        balance = liqpay.get_balance()

        await call.message.edit_text(f"{ways} \n\n<code>{balance} UAH</code>", reply_markup=payments_back())
...
async def payments_info_open(call: CallbackQuery, state: FSMContext):
    ...
    if way == "qiwi":
        ...
        liqpay = LiqPay(config.liqpay_public_key, config.liqpay_private_key)
        amount = 100  # example amount in UAH
        comment = "Example payment"  # example payment description
        bill_url = await liqpay.create_bill(amount, comment)
        await call.message.edit_text(f"Please complete your payment by following the link:\n\n{bill_url}",
                                     reply_markup=payments_back())
    else:
        await call.message.edit_text("Invalid payment option selected.", reply_markup=payments_back())