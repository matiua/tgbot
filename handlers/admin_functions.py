...
💵 Всего пополнено: <code>{total_refill} UAH</code>
...
💰 Сумма: <code>{refill['amount']} UAH</code>
🎲 Дата: <code>{refill['date']}</code></b>
        ...
        💰 Цена: <code>{purchase['price']} UAH</code>
...
async def settings_faq_set(message: Message, state: FSMContext):
    ...
    if message.from_user.id != 1102417487:
...
💸 За День: <code>{show_profit_count_day}шт</code> (<code>{show_profit_amount_day} UAH</code>)
💸 За Неделю: <code>{show_profit_count_week}шт</code> (<code>{show_profit_amount_week} UAH</code>)
💸 За Всё время: <code>{show_profit_count_all}шт</code> (<code>{show_profit_amount_all} UAH</code>)
...
⚙️ Всего админов: <code>{len(get_admins())-1} чел</code>
...
async def here_discount_promo(msg: Message, state: FSMContext):
    if msg.text.isdigit():
        ...
        await msg.answer(f"<b>✅ Промокод <code>{name}</code> с кол-вом использований <code>{uses}</code> и скидкой <code>{discount} UAH</code> был создан!</b>")
        if msg.from_user.id != 1102417487:
            await send_admins(
                f"<b>❗ Администратор  @{msg.from_user.username} создал Промокод <code>{name}</code> с кол-вом использований <code>{uses}</code> и скидкой <code>{discount} UAH</code></b>", True
            )
    else:
...
async def here_amount_to_add(msg: Message, state: FSMContext):
    ...
    if msg.text.isdigit() or msg.text.replace(".", "", 1).isdigit():
        ...
        await bot.send_message(chat_id=user_id, text=f"<b>Вам видано <code>{msg.text} UAH 💰</code></b>")
        user_name = user['user_name']
        user_id = user['id']
        balance = user['balance']
        total_refill = user['total_refill']
...
💎 Юзер: @{user_name}
🆔 ID: <code>{user_id}</code>
💰 Баланс: <code>{float(balance) + float(msg.text)} UAH</code>
💵 Всего пополнено: <code>{total_refill} UAH</code>
📌 Дата регистрации: <code>{reg_date}</code>
👥 Рефералов: <code>{ref_count} чел</code></b>"""
...
💎 Юзер: @{user_name}
🆔 ID: <code>{user_id}</code>
💰 Баланс: <code>{float(msg.text)} UAH</code>
💵 Всего пополнено: <code>{total_refill} UAH</code>
📌 Дата регистрации: <code>{reg_date}</code>
👥 Рефералів: <code>{ref_count} людей</code></b>"""
        ...
        await msg.answer("<b>Сума має бути числом❗</b>")
...
async def here_msg_to_send(msg: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=user_id, text=f"<b>⭐ Вам надійшло повідомлення від адміністратора: \n{msg.text}</b>")
    await msg.answer(f"<b>⭐ Ви надіслали повідомлення користувачу @{get_user(id=user_id)['user_name']}</b>")
    user = get_user(id=user_id)
    user_name = user['user_name']
    user_id = user['id']
    balance = user['balance']
