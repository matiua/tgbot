...
💵 Сумма: <code>{amount_pay} UAH</code>
...
@dp.callback_query_handler(text="complete_order")
async def complete_order(call: CallbackQuery):
    # удаляем сообщение с меню заказа
    await call.message.delete()

    # отправляем уведомление о выполнении заказа пользователю
    await bot.send_message(call.from_user.id, "Ваш заказ был выполнен.")

    # отправляем рекламное сообщение в телеграм-чат
    await bot.send_message(chat_id='https://t.me/SAKOSHOP_bot', text="Попробуйте наш бот! Он умеет многое.")
