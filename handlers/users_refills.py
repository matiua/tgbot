...
async def success_refill(call: CallbackQuery, way, amount, id, user_id):
    ...
    msg = f"💰 Произошло пополнение баланса! \n" \
          ...
          f"💵 Сумма пополнения: <code>{amount} UAH</code>\n" \
          f"🧾 Чек: <code>{id}</code> \n" \
        ...
        if min_amount <= pay_amount <= max_amount:
            if way == "crystal":
                ...
                params = {
                    ...
                    'currency': 'UAH',
                    'description': 'Оплата услуг бота',
                    'order_id': get_unix(),
                    'version': '3',
                    'result_url': f'https://t.me/{bot_name}?start=success_pay',
                    'server_url': f'https://{config.host}/{bot_name}/pay_liqpay'
                }
                link = liqpay.cnb_form(params)
                id = params['order_id']
...
async def check_refill(call: CallbackQuery, state: FSMContext):
    ...
    if way == "CrystalPay":
        ...
        if status:
            ...
            liqpay = LiqPay(config.liqpay_public_key, config.liqpay_private_key)
            status = await liqpay.check_payment(order_id=id)
            if status:
                await success_refill(call, way, amount, id, user_id)
            else:
                await call.answer(refill_check_no, True)
    elif way == "Lava":
        lava = Lava(shop_id=config.lava_project_id, secret_token=config.lava_secret_key)
